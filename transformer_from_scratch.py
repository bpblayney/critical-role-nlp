...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        """
        BB: I think head_dim can be the same as embed_size, just need different weights and initializations, and do copies..?
        :param embed_size: embedding is split into (8) different parts; original embed_size might be 256, which then splits into 8x32 parts.
        :param heads: heads is the number of parts (8) into which we split the embedding
        """
    ...
    def forward(self, values, keys, query, mask):
        N = query.shape[0] # How many examples sent at the same time
        value_len, key_len, query_len = values.shape[1], keys.shape[1], query.shape[1] # corresponds to source or target sentence length depending on which part of the model its used (encoder or part of decoder)
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_size, heads, dropout, forward_expansion):
        ...
        # Forward expansion of 4 used in paper: intermediate number of nodes
...
class Decoder(nn.Module):
    ...
    def forward(self, x, enc_out, src_mask, trg_mask):
        ...
        for layer in self.layers:
            # forward(self, x, value, key, src_mask, trg_mask)
            # value and key are the same thing and are output from the encoder
            # BB: shouldn't they end up different?
...
class Transformer(nn.Module):
    ...
    def make_src_mask(self, src):
        # if it is a self mask index set to 0 else set to 1
        src_mask = (src != self.src_pad_idx).unsqueeze(1).unsqueeze(2)
        # (N, 1, 1, src_len) ... just so the shapes add up
    ...
    def forward(self, src, trg):
        ...
        enc_src = self.encoder(src, src_mask) # these arguments go to the forward function of the encoder..
